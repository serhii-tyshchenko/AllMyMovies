export const ADD_ITEM = 'ADD_ITEM';
export const UPDATE_ITEM = 'UPDATE_ITEM';
export const REMOVE_ITEM = 'REMOVE_ITEM';
export const GET_ITEMS = 'GET_ITEMS';
export const SORT_MOVIES = 'SORT_MOVIES';
export const REMOVE_LIST = 'REMOVE_LIST';
export const SEARCH_MOVIE = 'SEARCH_MOVIE';
export const GET_MOVIE_INFO = 'GET_MOVIE_INFO';
export const SIGN_IN = 'SIGN_IN';
export const SIGN_OUT = 'SIGN_OUT';
export const SIGN_UP = 'SIGN_UP';
export const GET_SETTINGS = 'GET_SETTINGS';
export const UPDATE_SETTINGS = 'UPDATE_SETTINGS';
export const SHOW_NOTIFICATION = 'SHOW_NOTIFICATION';
export const HIDE_NOTIFICATION = 'HIDE_NOTIFICATION';

export const API_SEARCH_MOVIE_REQUEST_STARTED =
  'API_SEARCH_MOVIE_REQUEST_STARTED';
export const API_SEARCH_MOVIE_REQUEST_ENDED = 'API_SEARCH_MOVIE_REQUEST_ENDED';
export const API_GET_MOVIE_INFO_REQUEST_STARTED =
  'API_GET_MOVIE_INFO_REQUEST_STARTED';
export const API_GET_MOVIE_INFO_REQUEST_ENDED =
  'API_GET_MOVIE_INFO_REQUEST_ENDED';

export const DB_REQUEST_STARTED = 'DB_REQUEST_STARTED';
export const DB_REQUEST_ENDED = 'DB_REQUEST_ENDED';
